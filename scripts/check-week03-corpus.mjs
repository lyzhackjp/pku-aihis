...
for (const file of readdirSync(new URL('imports/', root)).sort()) {
  ...
  // Same JSONL parsing and validator as the corpus manager's importCorpusFile / replaceCorpus.
...
console.log(`Corpus import validation passed: ${summary.included_sources} files, ${total} records.`);
